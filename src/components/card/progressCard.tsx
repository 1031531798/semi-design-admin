            ...
            <div className={'h-full flex flex-col grow'}>
                <div className={'flex flex-row justify-between pb-2'}>
                    ...
                    <div className={'flex flex-row font-bold'}>
                        $<NumberAnimate className={'text-gray-100'} start={0} end={value} time={1000} formatter={numberSplit}  />
                        <span className={'ml-1 mr-1'}>/</span>
                        <span className={'text-gray-400'}>${numberSplit(total)}</span>
                    </div>
                </div>
                <Progress stroke={color} percent={getPercent} style={{height: '8px'}} ></Progress>
            </div>
        </div>
    )
}
